...
sum_list1([], 0).
sum_list1([A|Y], N) :- sum_list1(Y, N1), N is N1 + A.

not_member(_, [], true).
not_member(A, [A|_], false).
not_member(A, [_|Z], R) :- not_member(A,Z,R).



student_average_grade(Student, AverageGrade) :-
  findall(Grade, grade(Student, _, Grade), GradeList),
  length1(GradeList, GradeListLength),
  sum_list1(GradeList, GradeListSum),
  GradeListLength > 0,
  AverageGrade is GradeListSum/GradeListLength.

is_student_passed_exam(Student, IsExamsPassed) :-
  findall(Grade, grade(Student, _, Grade), GradeList),
  not_member(2, GradeList, IsExamsPassed).

get_student_average_grade_and_check_if_exams_passed(Student) :-
  student_average_grade(Student, AverageGrade),
  is_student_passed_exam(Student, IsExamsPassed),
  format('~w average grade is ~w. Is exams passed: ~w~n', [Student, AverageGrade, IsExamsPassed]).

students_average_grade_and_is_exams_passed([]).
students_average_grade_and_is_exams_passed([S|L]) :- get_student_average_grade_and_check_if_exams_passed(S), students_average_grade_and_is_exams_passed(L).

get_students_average_grades_and_check_if_exams_passed :- findall(Student, student(_, Student), L), students_average_grade_and_is_exams_passed(L).



count_subject_failed_students(Subject) :-
  subject(ShortSubject, Subject),
  findall(_, grade(_, ShortSubject, 2), FailedList),
  length1(FailedList, CountFailed),
  format('Count of students failed ~w: ~w~n', [Subject, CountFailed]).

count_subjects_failed_students([]).
count_subjects_failed_students([S|L]) :- count_subject_failed_students(S), count_subjects_failed_students(L).

get_subjects_failed_students_count :- findall(Subject, subject(_, Subject), L), count_subjects_failed_students(L).
...
max_grade_in_group([student_with_average_grade(_, Max)], Max).
max_grade_in_group([student_with_average_grade(_, Avg)|Tail], Max) :-
    max_grade_in_group(Tail, MaxTail),
    max1(Avg, MaxTail, Max).

filter_students_with_max_average(StudentGrades, MaxAverage, FilteredStudents) :-
    findall(
        student_with_average_grade(Student, AverageGrade),
        (member1(student_with_average_grade(Student, AverageGrade), StudentGrades), AverageGrade =:= MaxAverage),
        FilteredStudents
    ).

get_group_max_average_grades_students(Group) :-
  findall(Student, student(Group, Student), Students),
  findall(
      student_with_average_grade(Student, AverageGrade),
      (member1(Student, Students), student_average_grade(Student, AverageGrade)),
      StudentGrades
  ),
  max_grade_in_group(StudentGrades, MaxAverageGrade),
  filter_students_with_max_average(StudentGrades, MaxAverageGrade, FilteredStudents),
  format('Max average grade students in group ~w: ~w~n', [Group, FilteredStudents]).

get_groups_max_average_grades_students([]).
get_groups_max_average_grades_students([G|L]) :- get_group_max_average_grades_students(G), get_groups_max_average_grades_students(L).

groups_max_average_grades_students :- findall(Group, student(Group, _), L), sort(L, UniqueGroups), get_groups_max_average_grades_students(UniqueGroups).
